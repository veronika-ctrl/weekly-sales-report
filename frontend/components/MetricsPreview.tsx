        ...
        <table className={`w-full table-fixed ${isPdfMode ? 'text-[8pt]' : 'text-xs'} ${isPdfMode ? 'break-inside-avoid' : ''}`}>
          ...
          <thead>
            <tr className="bg-gray-200 border-b">
              ...
              <th className={`${isPdfMode ? 'py-0.5 px-1' : 'py-2 px-2'} text-center font-medium text-gray-900 bg-gray-200`} colSpan={1}>
                &nbsp;
              </th>
              <th className={`${isPdfMode ? 'py-0.5 px-1' : 'py-2 px-2'} text-center font-medium text-gray-900 bg-gray-200`} colSpan={2}>
                <span className="relative -left-6 whitespace-nowrap">Latest Week: {actualDateRange}</span>
              </th>
              <th className={`${isPdfMode ? 'py-0.5 px-1' : 'py-2 px-2'} text-center font-medium text-gray-900 bg-gray-200`} colSpan={3}>
                <span className="relative left-8 whitespace-nowrap">Year-to-date</span>
              </th>
            </tr>
            <tr className="bg-gray-200 border-b">
              <th className={`${isPdfMode ? 'py-0.5 px-1' : 'py-1 px-2'} text-right font-medium text-gray-900 bg-gray-200 ${isPdfMode ? 'tabular-nums' : ''}`}>Actual</th>
              <th className={`${isPdfMode ? 'py-0.5 px-1' : 'py-1 px-2'} text-right font-medium text-gray-900 bg-gray-200 ${isPdfMode ? 'tabular-nums' : ''}`} title={`Same week last year: ${lastYearDateRange}`}>
                Last Year
              </th>
              <th className={`${isPdfMode ? 'py-0.5 px-1' : 'py-1 px-2'} text-right font-medium text-gray-900 bg-gray-200 ${isPdfMode ? 'tabular-nums' : ''}`} title="Year-over-year: vs same week last year">
                Y/Y %
              </th>
              <th className={`${isPdfMode ? 'py-0.5 px-1' : 'py-1 px-2'} text-right font-medium text-gray-900 bg-gray-200 ${isPdfMode ? 'tabular-nums' : ''}`}>YTD Actual</th>
              <th className={`${isPdfMode ? 'py-0.5 px-1' : 'py-1 px-2'} text-right font-medium text-gray-900 bg-gray-200 ${isPdfMode ? 'tabular-nums' : ''}`}>YTD Last Year</th>
              <th className={`${isPdfMode ? 'py-0.5 px-1' : 'py-1 px-2'} text-right font-medium text-gray-900 bg-gray-200 ${isPdfMode ? 'tabular-nums' : ''}`}>YTD vs Last Year</th>
            </tr>
          </thead>
          <tbody>
            {METRIC_LABELS.map((label, index) => {
              ...
              return (
                <tr key={metricKey} className={`border-b border-gray-200 last:border-b-0 ${isPdfMode ? 'break-inside-avoid' : ''}`}>
                  ...
                  <td className={`${isPdfMode ? 'py-0.5 px-1' : 'py-1 px-2'} text-right text-gray-700 bg-gray-100 font-semibold ${isPdfMode ? 'tabular-nums' : ''}`}>
                  ...
                  <td className={`${isPdfMode ? 'py-0.5 px-1' : 'py-1 px-2'} text-right text-gray-700 bg-blue-100 ${isPdfMode ? 'tabular-nums' : ''}`}>
